...
public class AltaUsuario extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        lblCobros1.setText("Bienvenido a Finzamida ");
    ...
    private void btnRegistrarseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarseActionPerformed
        ...
        if (curp.isEmpty() || rfc.isEmpty() || nombre.isEmpty() || apellidoPat.isEmpty() || correo.isEmpty() || contrasena.isEmpty()) {
        ...
        try {
            ...
            String sqlUsuario = "INSERT INTO usuarios (CURP, RFC, Nombre, ApellidoPat, ApellidoMat, CorreoElectronico, Contrasena, FechaAlta) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            psUsuario = conexionTransaccion.prepareStatement(sqlUsuario, PreparedStatement.RETURN_GENERATED_KEYS);
            psUsuario.setString(1, curp);
            psUsuario.setString(2, rfc);
            psUsuario.setString(3, nombre);
            psUsuario.setString(4, apellidoPat);
            psUsuario.setString(5, apellidoMat.isEmpty() ? null : apellidoMat); // Permitir apellido materno vacío
            psUsuario.setString(6, correo);
            psUsuario.setString(7, contrasena);
            psUsuario.setDate(8, sqlFechaAlta);
            psUsuario.executeUpdate();

            rsUsuario = psUsuario.getGeneratedKeys();
            int idNuevoUsuario = -1;
            if (rsUsuario.next()) {
                idNuevoUsuario = rsUsuario.getInt(1);
            }

            if (idNuevoUsuario != -1) {
                String sqlCategoria = "INSERT INTO categorias (idUsuario, Nombre) VALUES (?, ?)";
                psCategoria = conexionTransaccion.prepareStatement(sqlCategoria);
                psCategoria.setInt(1, idNuevoUsuario);
                psCategoria.setString(2, "Alimentos");
