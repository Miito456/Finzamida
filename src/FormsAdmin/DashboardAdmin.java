...
public class DashboardAdmin extends javax.swing.JFrame {
    ...
    private void cargarGraficoCategorias() {
        ...
        String consulta = "SELECT c.Nombre, SUM(t.Monto) AS TotalPagado "
                ...
                + "WHERE t.Tipo = 'Egreso' AND t.Fecha >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) "
                + "GROUP BY c.Nombre "
    ...
    private void cargarSaldoTotal() {
        ...
        String consultaIngresos = "SELECT SUM(Monto) AS totalIngresos FROM transacciones WHERE Tipo = 'Ingreso' AND Fecha >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
        String consultaEgresos = "SELECT SUM(Monto) AS totalEgresos FROM transacciones WHERE Tipo = 'Egreso' AND Fecha >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
    ...
    private void cargarTotalIngresos() {
        String consulta = "SELECT SUM(Monto) AS totalIngresos FROM transacciones WHERE Tipo = 'Ingreso' AND Fecha >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
        try (PreparedStatement ps = reg.prepareStatement(consulta)) {
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    private void cargarTotalEgresos() {
        String consulta = "SELECT SUM(Monto) AS totalEgresos FROM transacciones WHERE Tipo = 'Egreso' AND Fecha >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
        try (PreparedStatement ps = reg.prepareStatement(consulta)) {
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
